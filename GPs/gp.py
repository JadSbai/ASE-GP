...
class GP:
    ...
    def get_kernel(self, params=None):
        if self.descriptor == "soap":
            ...
            # params = [10, 10, 1]
            periodic_kernel = GPy.kern.PeriodicExponential(
                input_dim=1, active_dims=[0], variance=params[0]
            )
            for dim in range(1, 20):
                periodic_kernel += GPy.kern.PeriodicExponential(
                    input_dim=1, active_dims=[dim], variance=params[0]
                )
            return (
                GPy.kern.RBF(
    ...
    def get_positions(self):
        if self.descriptor == "soap":
            (
                train_positions,
                validate_positions,
            ) = self.simulation.calculate_soap_descriptors()
            ...
            (
                train_positions,
                validate_positions,
            ) = self.simulation.calculate_distance_descriptors()
            ...
            for i in range(len(train_positions)):
                ...
                train_positions[i] = np.pad(
                    train_positions[i],
                    (0, self.feature_size - len(train_positions[i])),
                    "constant",
                )
            for i in range(len(validate_positions)):
                validate_positions[i] = np.array(validate_positions[i]).flatten()
                validate_positions[i] = np.pad(
                    validate_positions[i],
                    (0, self.feature_size - len(validate_positions[i])),
                    "constant",
                )
            ...
            train_positions = np.array(
                [at.get_positions().flatten() for at in self.simulation.train_positions]
            )
            validate_positions = np.array(
                [
                    at.get_positions().flatten()
                    for at in self.simulation.validate_positions
                ]
            )
    ...
    def get_space(self):
        if self.descriptor == "soap":
            ...
            return [Real(10, 1e2, name="periodic_variance")]
